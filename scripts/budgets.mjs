...
if (process.argv.includes('--image')) {
  ...
  execFileSync('docker', ['run', '--rm', '--entrypoint', '/bin/sh', 'logistics-os:local', '-ec', 'test "$(id -u)" != 0; for x in node npm go gcc; do ! command -v "$x"; done; test ! -d /src; test -f /etc/ssl/certs/ca-certificates.crt; test -d /usr/share/zoneinfo; test -z "$(find /var/cache/apk -type f)"']);
}
const report = {
  ...
  gzipLevel: 9, frontend: sizes, initialAssets: [...initial].sort(), image, services,
